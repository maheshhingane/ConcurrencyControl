/* contains member functions for the hash table */
...
//finds whether the obno,sgno in the hash table belongs to tid.
// returns the hash object if it belongs to the SAME tx
...
//  zgt_tm *tm;
	...
	{
		if ((linkp->obno == obno) && (linkp->sgno == sgno) && (tid == linkp->tid))
			return (linkp);
		else
			linkp = linkp->next;
...
// removes an object from the hash table. returns 0 if unsuccessful
